...
#define kLTGoodColor                  [UIColor colorWithRed:135.0 / 255.0 green:183.0 / 255.0 blue:74.0 / 255.0 alpha:1]
#define kLTAverageColor               [UIColor colorWithRed:214.0 / 255.0 green:161.0 / 255.0 blue:69.0 / 255.0 alpha:1]
#define kLTPoorColor                  [UIColor colorWithRed:187.0 / 255.0 green:63.0 / 255.0 blue:55.0 / 255.0 alpha:1]
#define kLTDefaultColor               [UIColor colorWithRed:54.0 / 255.0 green:57.0 / 255.0 blue:64.0 / 255.0 alpha:1]
#define kLTCpuIndicatorColor          [UIColor colorWithRed:71.0 / 255.0 green:74.0 / 255.0 blue:81.0 / 255.0 alpha:1]
#define kLTMonitorValueColor          [UIColor colorWithRed:188.0 / 255.0 green:188.0 / 255.0 blue:188.0 / 255.0 alpha:1]
#define kLTMonitorTitleColor          [UIColor colorWithRed:20.0 / 255.0 green:20.0 / 255.0 blue:20.0 / 255.0 alpha:1]
#define kLTMonitorBackgroundColor     [UIColor colorWithRed:39.0 / 255.0 green:42.0 / 255.0 blue:49.0 / 255.0 alpha:1]

static CGFloat const kLTMonitorViewWidth = 300;
static CGFloat const kLTMonitorViewHeight = 180;
static CGFloat const kRCStackViewHeight = 360;


@interface LTMonitorViewController ()
...
@implementation LTMonitorViewController
...
- (void)setupUI {
    _backView = [[UIView alloc] initWithFrame:CGRectMake(30, 80, kLTMonitorViewWidth, kLTMonitorViewHeight)];
    _backView.backgroundColor = kLTMonitorBackgroundColor;
    ...
    fpsTitleLayer.frame = CGRectMake((_backView.bounds.size.width - 80) / 2, kLTMonitorViewHeight * 0.08, 80, 15);
    ...
    fpsBackLayer.strokeColor = kLTDefaultColor.CGColor;
    ...
    _fpsArcLayer.strokeColor = kLTPoorColor.CGColor;
    _fpsArcLayer.fillColor = [UIColor clearColor].CGColor;
    
    CGPoint fpsArcCenter = CGPointMake(kLTMonitorViewWidth * 0.5, kLTMonitorViewHeight * 0.65);
    CGFloat fpsArcRadius = kLTMonitorViewHeight * 0.45;
    ...
    CGFloat indicatorHeight = fpsArcCenter.y - kLTMonitorViewHeight * 0.15;
    _cpuIndicatorLayer = [CAShapeLayer layer];
    _cpuIndicatorLayer.frame = CGRectMake(fpsArcCenter.x, kLTMonitorViewHeight * 0.15 + indicatorHeight / 2, indicatorWidth, indicatorHeight);
    ...
    _fpsTextLayer.frame = CGRectMake((_backView.bounds.size.width - 80) / 2, kLTMonitorViewHeight * 0.25, 80, 15);
    
    // CPU
    CATextLayer *cpuTitleLayer = [self createTitleTextLayerWithSuperLayer:_monitorBackView.layer];
    cpuTitleLayer.frame = CGRectMake(0, kLTMonitorViewHeight * 0.35, kLTMonitorViewHeight * 0.55, 15);
    cpuTitleLayer.string = @"CPU";
    
    CAShapeLayer *cpuBackLayer = [CAShapeLayer layer];
    cpuBackLayer.lineWidth = 2;
    cpuBackLayer.strokeColor = kLTDefaultColor.CGColor;
    ...
    _cpuArcLayer.strokeColor = kLTPoorColor.CGColor;
    _cpuArcLayer.fillColor = [UIColor clearColor].CGColor;
    _cpuArcLayer.lineCap = kCALineJoinRound;
    
    UIBezierPath *cpuFullPath = [UIBezierPath bezierPathWithArcCenter:CGPointMake(kLTMonitorViewHeight * 0.55 / 2, kLTMonitorViewHeight * 0.72) radius:kLTMonitorViewHeight * 0.45 / 2 - _cpuArcLayer.lineWidth / 2 startAngle:3.0 / 2.0 * M_PI endAngle:3.0 / 2.0 * M_PI + 2.0 * M_PI clockwise:true];
    ...
    _cpuTextLayer.frame = CGRectMake(0, kLTMonitorViewHeight * 0.65, kLTMonitorViewHeight * 0.55, 10);
    
    // Memory
    CATextLayer *memoryTitleLayer = [self createTitleTextLayerWithSuperLayer:_monitorBackView.layer];
    memoryTitleLayer.frame = CGRectMake(kLTMonitorViewWidth - kLTMonitorViewHeight * 0.55, kLTMonitorViewHeight * 0.35, kLTMonitorViewHeight * 0.55, 15);
    memoryTitleLayer.string = @"Memory";
    
    CAShapeLayer *memoryBackLayer = [CAShapeLayer layer];
    memoryBackLayer.lineWidth = 2;
    memoryBackLayer.strokeColor = kLTDefaultColor.CGColor;
    ...
    _memoryArcLayer.strokeColor = kLTPoorColor.CGColor;
    _memoryArcLayer.fillColor = [UIColor clearColor].CGColor;
    _memoryArcLayer.lineCap = kCALineJoinRound;
    
    UIBezierPath *memoryFullPath = [UIBezierPath bezierPathWithArcCenter:CGPointMake(kLTMonitorViewWidth - kLTMonitorViewHeight * 0.55 / 2, kLTMonitorViewHeight * 0.72) radius:kLTMonitorViewHeight * 0.45 / 2 - _cpuArcLayer.lineWidth / 2 startAngle:3.0 / 2.0 * M_PI endAngle:3.0 / 2.0 * M_PI + 2.0 * M_PI clockwise:true];
    ...
    _memoryTextLayer.frame = CGRectMake(kLTMonitorViewWidth - kLTMonitorViewHeight * 0.55, kLTMonitorViewHeight * 0.59, kLTMonitorViewHeight * 0.55, 25);
...
- (CATextLayer *)createTitleTextLayerWithSuperLayer:(CALayer *)superLayer {
    ...
    titleLayer.foregroundColor = kLTMonitorValueColor.CGColor;
...
- (CATextLayer *)createValueTextLayerWithSuperLayer:(CALayer *)superLayer {
    ...
    valueLayer.foregroundColor = kLTMonitorValueColor.CGColor;
...
- (void)setFPSValue:(CGFloat)fpsValue {
    ...
    if (fpsValue >= 57) {
        _fpsArcLayer.strokeColor = kLTGoodColor.CGColor;
    } else if(fpsValue >= 50) {
        _fpsArcLayer.strokeColor = kLTAverageColor.CGColor;
    } else {
        _fpsArcLayer.strokeColor = kLTPoorColor.CGColor;
    }
    ...
    _fpsArcLayer.path = [UIBezierPath bezierPathWithArcCenter:CGPointMake(_backView.bounds.size.width / 2, kLTMonitorViewHeight * 0.65) radius:kLTMonitorViewHeight * 0.45 - _fpsArcLayer.lineWidth / 2 startAngle:startAngle endAngle:endAngle clockwise:true].CGPath;
    _cpuIndicatorLayer.transform = CATransform3DMakeRotation(endAngle + M_PI / 2 + 4 / kLTMonitorViewHeight * 0.6, 0, 0, 1);
...
- (void)setCPUValue:(CGFloat)cpuValue {
    ...
    if (cpuValue <= 20) {
        _cpuArcLayer.strokeColor = kLTGoodColor.CGColor;
    } else if(cpuValue <= 80) {
        _cpuArcLayer.strokeColor = kLTAverageColor.CGColor;
    } else {
        //大于80%，此时应该获取当前线程的堆栈信息，上传服务器
        _cpuArcLayer.strokeColor = kLTAverageColor.CGColor;
    }
    
    _cpuTextLayer.string = [NSString stringWithFormat:@"%.2f%%", cpuValue];
    
    _cpuArcLayer.path = [UIBezierPath bezierPathWithArcCenter:CGPointMake(kLTMonitorViewHeight * 0.55 / 2, kLTMonitorViewHeight * 0.72) radius:kLTMonitorViewHeight * 0.45 / 2 - _cpuArcLayer.lineWidth / 2 startAngle:startAngle endAngle:endAngle clockwise:true].CGPath;
}

- (void)setMemoryValue:(CGFloat)memoryValue {
    if (_totalMemory == 0) {
        _totalMemory = [LTMemoryMonitor rcDeviceUsedMemory] + [LTMemoryMonitor rcDeviceAvailableMemory];
    }
    
    CGFloat startAngle = 3.0 / 2.0 * M_PI;
    CGFloat endAngle = startAngle + 2.0 * M_PI * memoryValue / _totalMemory;
    if (memoryValue <= 200) {
        _memoryArcLayer.strokeColor = kLTGoodColor.CGColor;
    } else if(memoryValue <= 280) {
        _memoryArcLayer.strokeColor = kLTAverageColor.CGColor;
    } else {
        _memoryArcLayer.strokeColor = kLTAverageColor.CGColor;
    }
    
    _memoryArcLayer.path = [UIBezierPath bezierPathWithArcCenter:CGPointMake(kLTMonitorViewWidth - kLTMonitorViewHeight * 0.55 / 2, kLTMonitorViewHeight * 0.72) radius:kLTMonitorViewHeight * 0.45 / 2 - _cpuArcLayer.lineWidth / 2 startAngle:startAngle endAngle:endAngle clockwise:true].CGPath;
...
- (void)backViewLongPressed:(UIGestureRecognizer *)recognizer {
    [[LTMonitorCenter defaultCenter] rcMonitorDisable];
}
...
- (void)leakStackViewTapped:(UIGestureRecognizer *)recognizer {
    ...
    if (stackViewframe.size.height == kLTMonitorViewHeight) {
        stackViewframe.size.height = kRCStackViewHeight;
        backViewFrame.size.height = kRCStackViewHeight;
        _leakStackView.scrollEnabled = YES;
    } else {
        stackViewframe.size.height = kLTMonitorViewHeight;
        backViewFrame.size.height = kLTMonitorViewHeight;
        _leakStackView.scrollEnabled = NO;
    }
    
    [UIView animateWithDuration:0.2 animations:^{
        self->_leakStackView.frame = stackViewframe;
        self->_backView.frame = backViewFrame;
    }];
}
...
#pragma mark- LTMonitorWindowDelegate
